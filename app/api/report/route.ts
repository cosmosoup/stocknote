...
import { loadPortfolio, saveReportLog, getHistoryData, getMacroStrategy } from "@/lib/supabase";
...
export async function POST() {
  try {
    ...
    // 2. 並行データ取得（市場データ・ニュース・履歴・マクロ戦略）
    const [market, news, history, macroStrategy] = await Promise.all([
      fetchMarketData(portfolio),
      fetchNews(),
      getHistoryData(30),
      getMacroStrategy(),
    ]);
    ...
    // 4. チャートURL生成
    const charts = buildCharts(market.portfolio, history);
